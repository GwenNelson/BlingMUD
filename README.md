...
Mudlet's bundled `generic_mapper` does not consume `Room.Info` automatically; by default it detects the prompt, room name, and exits from visible text. BlingMUD therefore prints exits in its recognized single-line form (`Exits: north east ...`) and accepts bare `look` and direction commands. In a new Mudlet profile, run `find prompt`, issue `look`, check that `map basics` shows both room name and exits, then run `start mapping BlingMUD`. `lua display(gmcp.Room.Info)` is a separate way to verify the GMCP feed.
