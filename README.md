...
GMCP is available as Telnet option 201. After `IAC DO GMCP`, clients may identify themselves with `Core.Hello` and subscribe with `Core.Supports.Set`, `.Add`, or `.Remove`. BlingMUD currently publishes subscription-aware `Char.Name`, `Char.Vitals`, `Char.StatusVars`, `Char.Status`, and `Room.Info`; it also answers `Core.Ping`. Messages and retained subnegotiations are capped at 16 KiB, subscriptions at 64 packages, and unchanged state snapshots are suppressed. Each production room has a permanent positive GMCP number in addition to its persistence-facing string `room_id`; released numbers must not be reassigned.

BlingMUD includes an installable Mudlet package at `mudlet/BlingMUD.xml`. It consumes authoritative `Room.Info`, creates and recenters rooms immediately without `find prompt` or `start mapping`, follows manually typed slash movement, and replaces double-click speedwalking with GMCP-confirmed `/n`, `/e`, and similar commands. Install it once through Mudlet's Package Manager (`Alt+O`) and reconnect; `mudlet/README.md` contains the exact setup and diagnostics. It replaces the bundled `generic_mapper` for this profile, which cannot observe slash-prefixed movement correctly because its movement capture runs client-side before BlingMUD receives the command.

The visible one-line `Exits: north east ...` output and finite bare movement conveniences remain as a fallback for clients or profiles that intentionally use the text-trigger generic mapper. `lua display(gmcp.Room.Info)` verifies the GMCP feed independently.
