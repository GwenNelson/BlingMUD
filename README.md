...
Rooms with no players receive no global NPC heartbeat. Active NPCs decide through one lazy finite-mailbox actor each; one stuck callback makes only that actor inert and never creates a replacement worker. Brave Sir Knight and Val use local FSM behavior, and the possum uses a simple local state machine. Optional OpenRouter work is explicitly disabled unless `BLINGMUD_OPENROUTER_ENABLED` is set. Wrapped NPCs start and reply in FSM mode; after the first validated provider response they report LLM mode and can emit a bounded in-character follow-up on a later actor tick. Their checked-in bounded persona templates, current speaker, and at most three recent validated exchanges provide conversational character and continuity; canned fallback sentences are withheld from speech prompts to prevent parroting. Any failure immediately returns reported mode to FSM while the local reply path remains available. Paid text models use a persistent one-dollar UTC-day budget before free-model fallback, with global/room/NPC request bounds.
