# BlingMUD Content and Free-Only NPC AI Roadmap
  ...
  `AdvisoryFSMBehavior` wraps an existing behavior without replacing its state, emits a bounded structured frame only in occupied rooms, and preserves an exact local decision for every remote attempt. Startup/no-provider speech emits that local decision. Once an NPC has a validated live response, successful speech emits only remote text; admission rejection returns local output immediately, provider failure queues it, and the five-second deadline releases it while rejecting late remote text. Global runtime disable or per-NPC local mode demotes reported mode and substitutes exact saved fallbacks for queued or in-flight remote speech. Explicitly enabled worlds wrap Brave Sir Knight and Val; disabled worlds construct no runtime and do not read the key. The current provider transport rejects redirects and insecure key files, validates text-only models from a bounded 2 MiB live catalogue, prefers inexpensive paid instruction models under a persistent one-dollar UTC-day budget, reserves prompt cost with a tokenizer-independent one-ASCII-byte-per-token ceiling, fails an over-limit persisted ledger closed at the cap, and falls back to validated free models.
  ...
  Advisory responses use validated choice indices for finite local candidate pools. Player-speech responses may also include bounded control-safe conversational text, which is queued and emitted only by the NPC actor on a later tick; stale, invalid, or unavailable results preserve the exact fallback decision. The implemented dialogue path loads the explicit bounded Knight/Val persona templates, includes the current speaker and at most three recent validated exchanges, and withholds canned candidate text from model-facing speech frames to prevent generic parroting. Val now supplies a bounded generic local candidate after her keyword-specific choices, so ordinary follow-up speech remains useful offline and is never silently excluded from live advisory requests.
  ...
  local/advisory mode for the wrapped Knight and Val, plus metadata-only NPC
  state counts. Template reload and raw memory/brain inspection remain
