...
- [done: current inspection] `/adminstatus` shows bounded connection/session/schema/NPC/persistence/status counters; `rooms` lists at most 20 room activity records and `npcs` lists at most 20 NPC room/mode/fallback/queue/error records. `/adminai` adds bounded provider status, refresh, cooldown-clear, runtime admission, wrapped-NPC local/advisory controls (including `enable knight advisory` syntax), and metadata-only NPC state inspection. Admin output contains no auth material, provider secrets, or memory text, and all six admin commands are hidden from non-admin help/completion.
