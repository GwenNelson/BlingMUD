...
- OpenRouter is now explicitly authorized and implemented for bounded NPC dialogue: the adapter rejects redirects and insecure keys, validates the live catalogue within a 2 MiB bound, selects preferred inexpensive instruction models under a persistent one-dollar UTC-day budget before free fallback, uses correct per-million-token routing limits, and runs through a finite runtime with global, room, and NPC request budgets. Each wrapped NPC starts in FSM mode, independently switches to reported LLM mode only after its own validated response, and reverts immediately on failure. Brave Sir Knight produces an immediate local reply and may emit a validated, bounded in-character OpenRouter follow-up on a later actor tick. Full raw provider payload/response audits go only to the ignored owner-only bounded audit file, never ordinary logs or authorization data; capacity is reserved before each request, so audit exhaustion fails closed to the local FSM without an unlogged provider call.
- Meaningful dialogue now uses the bounded checked-in Knight and Val persona templates rather than the former generic runtime prompt. Speech frames identify the current speaker, carry at most three recent validated exchanges for short conversational continuity, and omit canned candidate text from the model-facing JSON so the model composes a specific reply instead of parroting FSM filler. The internal finite candidate slots, local state, and all mechanics remain authoritative. Knight's immediate local fallback also recognizes explicit farewell and departure intent.
