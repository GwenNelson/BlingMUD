...
- [remaining fidelity] Add actual food and prices/economy, bounded regular/recent-order/bad-behaviour memory, exhaustion and bloodied-gear signals, richer intoxication side effects, durable offline decay in schema version 2, and safe custom drink payloads with names/descriptions/provenance. The current impossible request becomes a fixed `horn-born special`; cosmetic duplicate service does not create multiple Val NPCs.
...
- Health/status tests cover input validation, health/intoxication clamps, the exact half-health injury boundary, non-destructive collapse state and relocation, whole-minute online decay, and backwards-clock safety. Offline elapsed-time handling remains a version-2 persistence test target.
...
- Remaining drink work: define a bounded serializable custom-drink schema, deterministic concept normalization, allowed effect tables, richer intoxication status effects, offline decay, Acorn Goblet integration, food, and a real price/economy layer. Never persist raw model output or allow a generated concept to select classes, code, unbounded text, or arbitrary stat changes.
