...
- OpenRouter is now explicitly authorized and partly implemented: the optional adapter rejects redirects and insecure keys, validates the live catalogue within a 2 MiB bound, accepts only bounded free text models, rotates with cooldown fallback, and runs through a finite advisory runtime. Brave Sir Knight and Val remain local-authoritative wrappers. Live catalogue refresh succeeds with the configured key; live completion can exhaust the current free pool and falls back locally. Remaining local work is budgeted scheduling, structured-choice refinement, persistence, and admin controls.
