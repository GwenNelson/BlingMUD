...
- OpenRouter is now explicitly authorized and partly implemented: the optional adapter rejects redirects and insecure keys, validates the live catalogue within a 2 MiB bound, selects text models under a hard one-dollar daily paid reservation budget before falling back to free models, rotates with cooldown fallback, and runs through a finite advisory runtime with global, room, and NPC request budgets. Brave Sir Knight and Val remain local-authoritative wrappers with choice-index hints and durable bounded snapshots. Live catalogue refresh succeeds with the configured key; live completion can exhaust available pools and falls back locally. Remaining local work is richer admin memory/reload tooling and the remaining village content.
