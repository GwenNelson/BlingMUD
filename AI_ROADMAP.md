...
- [done: event primitive] Tab produces a bounded `TabInputEvent` carrying the current input instead of terminating the line. `Session` receives it on its sequential worker. Command-aware candidate selection and safe input replacement are the next command-specification stage.
...
- Still planned:
  - command-aware help and actual completion/input replacement layered on the implemented incremental parser's Tab events
