...
- OpenRouter is now explicitly authorized and partly implemented: the optional adapter, strict free-only catalogue filter, rotation/cooldowns, bounded advisory runtime, empty-room gate, and Knight/Val local-fallback wrappers are present. Live catalogue verification is externally unavailable at the latest bounded attempts; remaining local tests, administration, prioritization, and structured-choice refinement must continue independently.
