...
- Implemented now: the first-fix engine bugs above, prominent plaintext-Telnet warnings at startup and before authentication, deliberate hidden-input echo/redraw suppression, selector-owned sockets/pre-auth/timeouts/output, bounded authentication workers/rate limits/finite shutdown, no-listener socketpair and fake-clock integration coverage, one sequential gameplay worker per authenticated player, one shared incremental Telnet/UTF-8 parser with safe command-token completion, bounded subscription-aware GMCP Core/character/mapper output, validated global/room command specs and generated help, room-first ordinary dispatch with protected safety/admin names and duplicate-registration rejection, item/slot-aware `/unequip` with `/remove` alias, bounded `/tell` and `/hug`, admin-only confirmed shutdown/output-preserving kick/shared-API heal/focused-or-bulk save/bounded status views, bounded JSON-lines operational events for authentication/connection/server/admin/persistence/room/NPC metadata with conservative redaction and exception-type-only failures, 60-second dirty-only character and shared-world autosave, separate bounded/coalescing character, world, and NPC-state writers with final saves inside the ten-second graceful deadline, callback-complete queue rejection, transactional SQLite schema migrations through version 4, bounded character JSON v3 with v2/v1/legacy migration and nine explicit item templates, durable recent-collapse/status timestamps, coins, Goblet contents, and safe offline intoxication decay, strict version-1 world and NPC JSON, matching gameplay inventory/room-item limits, centralized bounded damage/healing/intoxication APIs, one-damage falling acorns, five-damage Val cats, non-destructive zero-health collapse/return to Town Square, recent-collapse recognition by Val, non-blocking one-point-per-minute online intoxication decay, NPC-manager removal cleanup, explicit room activity snapshots, global hot-room-only NPC heartbeat selection, restartable/event-stoppable ticker lifecycle, per-NPC bounded actor isolation with inert timeout fallback and no replacement workers, the shared NPC behavior/event-dispatch contract, Unicode-control-safe structured NPC actions, reusable local random and data-backed FSM behaviors, Brave Sir Knight's migration onto `FSMBehavior`, bounded non-repeating dialogue selection, detached-room tick safety, the stateful Suspicious Alley bin-possum encounter, the first Village Green/Hanging Tree/Wisp Mother slice, the initial Val's Hella Holler/Val/fixed-drink slice, the Temple of the Self safe-room slice, Smithereens/Eisele/Tackdriver initial content, Ceridwen's cottage/garden initial content, and regression coverage for all of them.
- [done] Register direct northeast/northwest/southeast/southwest movement commands and `ne`/`nw`/`se`/`sw` aliases so every advertised diagonal content exit works without requiring `/go`.
- [done] Make bare `/look` and `/l` always use the full global room renderer, fixing the Temple's targeted mirror hook swallowing its ordinary description and exits while preserving targeted room-local look commands.
- [done: public-world audit] Add one integration contract spanning all 12 rooms, every reciprocal/directly executable exit, bare `/look` and `/l`, every advertised room command and alias, and a representative valid invocation of every room feature.
- Still planned: richer status effects, renewable but bounded village ecology, advanced admin memory/reload tools, richer custom horn drinks/food/prices/regulars, and the remaining village content from the email threads. The advisory wrapper now supports one-use choice-index hints for approved local candidate pools, Knight traveller memory is structured and capped at 64 entries, and explicit version-1 Knight/Val snapshots persist through schema v4. Corbel's local-FSM turnery, fixed acorn trade/crafting, bounded coins, persistent Goblets/Mash, and Val goblet filling are implemented; production multi-process/database identity verification remains outstanding. TLS and encrypted transport are explicitly excluded from this implementation plan; the residual plaintext risk is accepted and must remain honestly documented.
...
- OpenRouter is now explicitly authorized and implemented for bounded NPC dialogue: the adapter rejects redirects and insecure keys, validates the live catalogue within a 2 MiB bound, selects preferred inexpensive instruction models under a persistent one-dollar UTC-day budget before free fallback, uses correct per-million-token routing limits, and reserves prompt cost with a tokenizer-independent one-ASCII-byte-per-token ceiling. A persisted ledger at or above the cap fails closed rather than reopening paid spend. The finite runtime has global, room, and NPC request budgets. Each wrapped NPC starts in FSM mode and independently switches only after its own validated response. Once live, speech withholds its simultaneously computed FSM sentence: success queues only remote speech, admission rejection returns local output immediately, provider failure queues the exact fallback, and a five-second deadline releases fallback while rejecting late model text. Full raw provider payload/response audits go only to the ignored owner-only bounded audit file, never ordinary logs or authorization data; capacity is reserved before each request, so audit exhaustion fails closed without an unlogged provider call.
- Meaningful dialogue uses the bounded checked-in Knight and Val persona templates rather than the former generic runtime prompt. Speech frames identify the current speaker and FSM state, carry at most three recent validated exchanges for short conversational continuity, and omit canned candidate text from the model-facing JSON so the model composes a specific reply instead of parroting FSM filler. The internal finite candidate slots, local state, and all mechanics remain authoritative. Startup/no-provider FSM speech still recognizes explicit farewell and departure intent.
...
- Still planned in this layer: additional validated action types as game mechanics require them, richer optional structured memory, and activation/deactivation events.
- [done: local and advisory behavior stage] Extract a generic NPC behavior contract from the existing Brave Sir Knight implementation and provide reusable simple-random and deterministic FSM modes plus the fallback-preserving `llm_fsm` advisory wrapper.
...
- [done: bounded dialogue slice] Make LLM NPCs an optional wrapper, not the authority: the provider returns a structured candidate choice and bounded speech reply, while the engine keeps state and gameplay effects local and emits validated text only through the NPC actor.
...
- [done] Use a central bounded advisory runtime in the server process so gameplay threads never block directly on OpenRouter calls.
- Use catalogue-validated preferred model identifiers for reliable inexpensive dialogue, then deterministically consider other price-bounded catalogue entries and free models; never trust an identifier without current pricing and modality checks.
...
- Re-enable each NPC's reported LLM mode only after that NPC receives a complete schema-valid response; startup, provider errors, timeouts, and invalid responses leave it in FSM mode.
- Require a structured JSON envelope. A bounded control-safe conversational reply may be accepted for speech, but item creation, movement, damage, state changes, and other gameplay effects remain validated local actions rather than freeform provider authority.
...
- [done: migration layer] `player_state.py` owns strict version-3 JSON for room ID, fabulousness, maximum/current health, intoxication, bounded coins, recent-respawn state, a bounded wall-clock status timestamp, inventory, equipment, and whitelisted Goblet contents. Version-2, version-1, and legacy `{}` documents migrate in memory with safe defaults. Current restores subtract one intoxication point per whole offline minute, clamp at zero, and preserve a future timestamp when the wall clock moves backwards.
- [done: database migrations] SQLite uses transactional `PRAGMA user_version` migrations through schema version 4. The migration creates/preserves the account, world-state, and explicit NPC-state tables idempotently, repairs only unambiguous stale canonical account keys, keeps existing account rows, and refuses collisions or a database from a newer runtime rather than downgrading it.
...
- [done: GMCP layer] Telnet option 201 uses the shared parser and bounded authenticated input queue. The server handles bounded `Core.Hello`, `Core.Supports.Set/Add/Remove`, `Core.Ping`, and `Core.KeepAlive`, then publishes only subscribed `Char.Name`, `Char.Vitals`, `Char.StatusVars`, `Char.Status`, and mapper-compatible `Room.Info`. Messages are capped at 16 KiB, subscriptions at 64, unchanged snapshots are suppressed, and permanent numeric GMCP room IDs remain separate from persistence IDs.
- [done: mapper command compatibility] Slash commands remain canonical, while exact bare `look`/`l` and the existing cardinal, diagonal, up, and down direction names/aliases dispatch the same commands expected by Mudlet's generic mapper and speedwalks. Other bare text remains chat, with reserved `/say` available for literal mapper-word speech.
- [done: generic mapper room detection] Visible exits use Mudlet generic mapper's recognized one-line `Exits: north east ...` form. The bundled generic mapper remains text-trigger-driven by default, while GMCP-aware profile scripts can independently consume the validated `Room.Info` feed.
- [done: native Mudlet GMCP mapper] The installable `mudlet/BlingMUD.xml` package replaces generic mapper heuristics for a BlingMUD profile. It requests `Room`, automatically creates, links, and recenters namespaced rooms from validated `Room.Info`, follows manual slash movement, and supplies bounded slash-prefixed speedwalking that advances only after the expected GMCP room confirmation.
...
- [done: dispatch precedence] Ordinary `Room.on_command` handlers run before global fallbacks, including room aliases, and room specs take matching help/completion precedence. The reserved names `admin`, `shutdown`, `kick`, `heal`, `save`, `adminstatus`, `adminai`, `say`, `quit`, and `exit` bypass rooms even if room code tries to claim them.
...
- [partly done: Ceridwen] Ceridwen's cottage, herb-garden rooms, and local FSM are present with bounded gated remedy/weed interactions. Rare Weed is now an explicit persisted template with one-at-a-time harvest, and garden examination changes once its finite patch is bare; giving the weed unlocks a runtime-local experimental shelf selling the existing bounded HornBornSpecial for three coins. Post-unlock salve, antitoxin, and repeated-give feedback reflects the actual shelf state without claiming unfinished remedies exist. Durable unlock and disorientation mechanics remain.
- [done: Ceridwen command dispatch] The advertised `/give weed` path claims missing and invalid arguments with useful rare-weed guidance instead of leaking through global dispatch as an unknown command.
- [done: tavern interaction] `/flirt val` remains a bounded Val-specific room command with local response behavior. Val's speech FSM also has a bounded generic final candidate, so ordinary follow-up conversation works locally and always gives live advisory mode a validated reply slot instead of silently skipping OpenRouter.
...
- [done: current inspection] `/adminstatus` shows bounded connection/session/schema/NPC/persistence/status counters; `rooms` lists at most 20 room activity records and `npcs` lists at most 20 NPC room/effective-mode/local-fallback/actor-fallback/queue/error records. Healthy actors report `actor_fallback=none` rather than the misleading Python `None`, while a callback timeout reports `actor_fallback=inert` without hiding the NPC's guaranteed local behavior. `/adminai` adds bounded provider status, refresh, cooldown-clear, runtime admission, wrapped-NPC local/advisory controls (including `enable knight advisory` syntax), and metadata-only NPC state inspection. `disable <npc>` unambiguously selects local mode and rejects a contradictory third argument. Disabling the runtime or forcing one NPC local demotes its reported mode and replaces queued or in-flight remote speech with the exact saved FSM fallback. Admin output contains no auth material, provider secrets, or memory text, and all admin-only commands are hidden from non-admin help/completion.
...
- [done: goblet feedback] Drinking a Val-filled Acorn Goblet preserves the contained fixed drink's concrete healing/intoxication message before clearing the drink, while retaining the reusable vessel.
- [done: initial drink mechanics] `/order` maps healing concepts, ordinary alcohol concepts, and all other bounded concepts to three fixed concrete persistent items. `/drink` applies clamped health/intoxication effects through the central player APIs, consumes successful drinks, preserves alcohol refused at maximum intoxication, and leaves healing potion available at maximum intoxication. Online intoxication now decays by one point per whole minute through non-blocking selector maintenance. Val deliberately keeps current drinks on the house; bounded coins now exist for Corbel's separate economy.
...
- Phase 1: [done] fix the concrete engine/security bugs, deliver versioned player persistence through current version 3, save/load on login and disconnect, and add bounded dirty-only periodic autosave.
...
- The implemented persistence tests now cover room/inventory/equipment/stat round trips including health, maximum health, intoxication, coins, all nine item templates, and bounded Goblet contents; legacy empty state and version-1/version-2 migrations; corrupt/oversized/unknown/inconsistent state rejection; safe room fallback; versioned new accounts; login restoration; logout ordering; failed-save preservation; session-only admin state; bounded input; and bounded password work. General status effects remain unimplemented and therefore are still a future test target.
...
- [done] Add explicit player-state migrations: legacy empty data, version 1, and version 2 now restore through strict version 3 with durable status timing and bounded coins.
