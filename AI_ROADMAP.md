...
- [partly done: Ceridwen] Ceridwen's cottage, herb-garden rooms, and local FSM are present with bounded gated remedy/weed interactions. Rare Weed is now an explicit persisted template with one-at-a-time harvest; giving it unlocks a runtime-local experimental shelf selling the existing bounded HornBornSpecial for three coins. Post-unlock salve, antitoxin, and repeated-give feedback reflects the actual shelf state without claiming unfinished remedies exist. Durable unlock and disorientation mechanics remain.
