...
class CeridwenGardenTests(unittest.TestCase):
    ...
    def test_cottage_and_garden_are_bounded_gated_slices(self):
        ...
        garden.on_command(session, "examine", "garden")
        self.assertIn("harvested patch", session.messages[-1])
        self.assertIn("now bare", session.messages[-1])
        self.assertNotIn("rare weed glows", session.messages[-1])
        cottage.enter(player, announce=False)
